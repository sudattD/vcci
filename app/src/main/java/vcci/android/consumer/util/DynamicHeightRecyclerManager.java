package vcci.android.consumer.util;
 ...
 * {@link #(int)} method might be used to let the layout manager know how big they are going to be.
 * If animations are not used at all then a normal measuring procedure will run and child views will be measured during
 * the measure pass.
 */
public class DynamicHeightRecyclerManager extends LinearLayoutManager {


    private final int[] mMeasuredDimension = new int[2];

    public DynamicHeightRecyclerManager(Context context) {
        super(context);
    }

    public DynamicHeightRecyclerManager(Context context, int orientation, boolean reverseLayout) {
